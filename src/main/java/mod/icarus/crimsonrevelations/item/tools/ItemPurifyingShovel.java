...
import mod.icarus.crimsonrevelations.item.base.ItemShovelBase;
...
public class ItemPurifyingShovel extends ItemShovelBase {
    ...
    public EnumActionResult onItemUse(@Nonnull EntityPlayer player, @Nonnull World world, BlockPos pos, @Nonnull EnumHand hand, @Nonnull EnumFacing facing, float hitX, float hitY, float hitZ) {
                ...
                for (int zee = pos.getZ() - 5; zee < pos.getZ() + 6; zee++) {
                    ...
                    if (target != null && target == BlocksTC.fluxGoo) {
                        ...
                        for (int k = 0; k < world.rand.nextInt(2); ++k) {
                            PacketHandler.INSTANCE.sendToAllAround(new PacketFXArcBolt(new Vec3d(player.posX, player.posY + 1.0D, player.posZ), new Vec3d(aim.getX(), aim.getY(), aim.getZ()), 12751838, 0.5F * 0.66F), new NetworkRegistry.TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 64.0D));
                        }
                    }
                }
            }
        ...
        if (purified > 0) {
            player.getHeldItem(hand).damageItem(Math.min(purified, ConfigHandlerNCR.purifying_shovel.cleanseMaxCost), player);
            player.swingArm(hand);
